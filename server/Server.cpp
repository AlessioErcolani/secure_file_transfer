    ...
    else 
    {
        ...
        try
        {
            ...
            {  
                ...
                case RECEIVE_PUB_KEY_CLIENT:
                    on_recv_public_key_client(sd, buffer, n);
                    break;        

                case RECEIVE_CERTIFICATE_CLIENT:
                    on_recv_certificate_client(sd, buffer, n);
                    break;
        ...
        {
            Log::e(e.what());
            //authentication_error_clear_information(sd);
            //TODO:onclose
        }
        delete[] buffer;
    }
...
void 
Server::
on_recv_public_key_client(int sd, byte buffer[], size_t n)
{
    ...
    if(!is_expected_size(key_len, n))
    {
        Log::e(TO_STR("invalid size: key_len = " << key_len << " n = " << n));
        throw security_exception("invalid size");
    }
    ...
    size_t msg_len = signature_len + digest_len + key_len;
    ...
    //Y_server||SIGN_server(Y_a||Y_b)||HMAC_sessionKey(SIGN)
    byte* msg_to_client = new byte[msg_len];
    memcpy(msg_to_client, pub_key, key_len);
    memcpy(msg_to_client + key_len, signature, signature_len);
    memcpy(msg_to_client + key_len + signature_len, computed_digest, digest_len);
...
void 
Server::
on_recv_certificate_client(int sd, byte recv_certificate[], size_t certificate_len )
{
    Log::dump(TO_STR("Message 3 (" << certificate_len << " bytes)").c_str(), recv_certificate, certificate_len);

    SessionInformation* session = &connection_information[sd];

    sendToHost(sd, (unsigned char*)CERT_RECV, sizeof(CERT_RECV));
    ...
    X509* client_certificate = cast_certificate_from_DER_format(recv_certificate, certificate_len);
...
on_recv_signature_hmac(int sd, byte buffer[], size_t buffer_len)
{
    ...
    Log::dump(TO_STR("Message 5 (" << buffer_len << " bytes)").c_str(), buffer, buffer_len);
    ...
    if (!session->file_manager->createDirectory(certificate_subject))            
        throw runtime_error("problem in create client directory");           
...
on_send_name_file(int sd, string file_name)
{
    ...
    string message_to_client = file_name;
    code = ACK_CODE_SEND;
    ...
    if (file_already_present)
    {
        ...
        if (!success)
            {
                code = ERROR_CODE;
                message_to_client = "Error on deleting the old version of the file";
            }  
    ...
    if (success)
    {
        ...
        catch(exception& e)
        {
            Log::e(e.what());
            code = ERROR_CODE;
            message_to_client = "Error on opening the file";
        }
    }

    pt_len = message_to_client.length()+1;
    pt = new byte[pt_len];
    memcpy(pt, message_to_client.c_str(), pt_len);
...
on_send_file_chunck(int fd, byte* buffer, size_t buffer_len)
{
    ...
    catch(exception& e)
    {
        Log::e(e.what());
        recovery(fd);
        session_clear_information(fd);
        connection_information.erase(fd);
        AbstractHost::onDisconnection(fd);
    }
...
{
    SessionInformation* session = &connection_information.at(fd);
    on_send_file_chunck(fd, buffer, buffer_len);    
...
on_delete_file(int sd, string file_name)
{
    ...
    if (!success)
    {
        ...
        {
            code = ERROR_CODE;
            body_msg = "problem in deleting file";            
        }
        else
        {
